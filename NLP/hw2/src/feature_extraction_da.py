...
def get_features_da(config,sent_dict):
    ...
    if len(config[0]) > 0:
        ...
        if len(config[1]) > 1:
            trd_stk = config[1][-2]
            trd_stk_token_feature = 'TRD_STK_TOKEN_'+str(sent_dict['FORM'][trd_stk].lower())
        else:
            trd_stk = 'NA'
            trd_stk_token_feature = 'TRD_STK_TOKEN_'+str(trd_stk)
        features.append(snd_stk_token_feature)
        
        # 12. POS of Second of Stack
        if len(config[1]) > 1:
            trd_stk = config[1][-2]
            trd_stk_pos_feature = 'TRD_STK_POS_'+str(sent_dict['POSTAG'][trd_stk].lower())
        else:
            trd_stk = 'NA'
            trd_stk_pos_feature = 'TRD_STK_POS_'+str(trd_stk)
        
        features.append(trd_stk_pos_feature)
    return features
